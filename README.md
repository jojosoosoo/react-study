...
(하나의 이름으로) 여러 개의 데이터와 여러 개의 기능을 묶어서
마치 데이터 종류처럼 사용할 수 있음

객체를 만든다는 말은 **데이터를 설계하는 것**임
...
객체는 다양한 자료를 묶어서 이름 한 개로 표현 가능하다는 점에서 좋음

JS에서 객체를 만드는 두 가지 형태 (리터럴, 함수)

1. 객체 리터럴

장점: 가독성이 좋음, 손만 고생하면 됨

단점: CRUD(추가, 수정 삭제)가 힘듦
...
개발자가 함수의 모양만 봐도 객체 생성 함수인지, 일반 함수인지 구별이 가능

Swiper( )
Axios( ) : 대문자 함수 → 객체 생성 함수

일반적으로 call하는 게 아니라는 걸 표현한다면

new Swiper( )
new Axios( )

2.2 함수 안쪽에 this는 call 방식에 따라 값이 다름
...
const go = new Foo()
go.goto(); // 여기서 this는 . 앞의 것 (go)를 가르킴

2.3 함수 call과 new 객체 생성자 함수를 구분하여 리턴하여야 함 (용도 구분 필수)

: 일반 함수는 자유롭게 return 값;

: 객체 생성용 함수는 제발 return 적지 말 것

2.4 constructor 함수 / non-constructor 함수

: 모든 함수는 new 사용 가능 (constructor)

: 그런데 ES6 도입 후 new 사용 못하는 함수 (화살표 함수, 축약 메서드 함수 non-constructor)

2.5 new를 통해 생성된 객체(인스턴스)의 원본 확인 연산자 instanceof
...
2. 리액트 용 변수 즉, useState 이해
  ...
  - 하면 안 되는 방식
    ...
    get이름 = 값을 직접 입력은 반영 안됨
    mbMenuOpen = true;  // 이러면 반영 안됨
...
- prettier 설정 (문서 포맷 정리)
  : 들여쓰기, 따옴표, 세미콜론 등등...
  `npm install --save-dev --exact prettier`
  : 환경 설정 파일을 /에 만듦
  : 파일명은 무조건 `.prettierrc.json`
...
- eslint & prettier 묶어서 설정
  : eslint에서 prettier의 설정을 안내해 주도록 연결
...
2. 클로저
  ...
  - JS는 함수를 호출 즉 Call 이 아니고, 작성을 하는 순간 모든 참조가 결정된다. (렉시컬 범위가 셋팅 완료)
...
4. 클로저의 조건
  ...
  4.4 외부 함수보다 중첩 함수가 더 오래 유지되는 경우
      중첩 함수는 이미 종료되어버린 외부 함수의 변수를 참조할 수 있음
  function Out () {
    let age = 10;
			function In () {
        return age;
...
6. 클로저란?
  ...
  6.3 이렇게 하면 외부에서 데이터에 접근할 수 없으므로, 데이터 은닉 가능

  6.4 데이터에 접근하는 내부 함수를 통해서만 접근 가능하므로 안전
...
8. 클로저
  ...
  8.1 데이터를 안전하게 보관하기 위한 목적
      즉, 외부에서 함부로 접근하지 못하게 함

  8.2 데이터를 변경해야 할 경우, 내부 함수로 변경하는 것
