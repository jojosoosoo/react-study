...
: 상식. .gif는 여러 장의 이미지를 일정한 시간으로 교체하면서 보여주는 파일

- `<img src="경로/파일명.확장자" />`
- `<img src="경로/파일명.확장자" alt="이미지 설명" />`

### 1.3. 목록 태그 (`ul, li`)

- 필수임
- 판단했을 때 레이아웃이면 div, 내용이 동일한 형태면 ul, li
...
- 유지, 보수 프로젝트는 `display: inline, display: inline-block`을 조심.
...
- 초기 디자인 및 css 작업은 글꼴에 대한 협의가 끝나고 진행 (1순위)
